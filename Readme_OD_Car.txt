...
Contains all the simulation related file which includes simulation for 3 streets(Mörfelder Landstrasse, Kennedyallee and Darmstädter Landstrasse)
It contains Origin Destination SuMO readable files with type OD File.
Traffic assignment zone file with an extention .taz are also mentioned. These two files are mentioned in the configuration file starting with the name "od2trips_"
...
Configuration File: duarcfg_file.trips2routes.duarcfg used to create route file containing routes for each of the trip files.
...
route files are stored with extension ".odtrips.rou"
Network_File : imported from OSM
